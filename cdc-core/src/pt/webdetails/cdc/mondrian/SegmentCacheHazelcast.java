...
public class SegmentCacheHazelcast implements SegmentCache {
  ...
  /**
   * Notifies the listener of what already is in cache.<br/> It does so by sending an external add event for each header
   * in cache.<br/> All relevant RolapStars must have been initialized inside mondrian for this to work.
   *
  ...
  static class SegmentCacheListenerWrapper implements EntryListener<SegmentHeader, SegmentBody> {
    ...
    private SegmentCacheListener.SegmentCacheEvent getSegmentCacheEvent(
        final EntryEvent<SegmentHeader, SegmentBody> event ) {
      return new SegmentCacheListener.SegmentCacheEvent() {

        public boolean isLocal() { //for this to work we have to make sure cache cleaning made directly through cdc
          // doesn't bypass mondrian api
