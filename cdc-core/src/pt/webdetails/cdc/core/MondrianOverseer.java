  ...
  public void registerMondrianCacheSpi( boolean addDebugger ) {
    ...
    for ( SegmentCache cache : SegmentCacheInjector.getCaches() ) {
      if ( cache instanceof SegmentCacheHazelcast ) {
        ...
        logger.fatal( "Found orphan " + SegmentCacheHazelcast.class.getName()
            + " in mondrian caches, please restart server or Mondrian will not work properly" );
  ...
  static class MondrianLoggingListener implements SegmentCacheListener {
    ...
    public void handle( SegmentCacheEvent event ) {
      switch ( event.getEventType() ) {
        case ENTRY_CREATED:
          logger.debug( "(" + ( event.isLocal() ? "local" : "remote" ) + ") " + "Mondrian cache entry ADDED: " + event
              .getSource() );
          break;
        case ENTRY_DELETED:
          logger.debug( "(" + ( event.isLocal() ? "local" : "remote" ) + ") " + "Mondrian cache entry REMOVED: " + event
              .getSource() );
          break;
      }
    }
