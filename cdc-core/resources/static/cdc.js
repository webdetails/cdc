...
cdcFunctions.generateLink = function(){
  ...
  var path = window.location.protocol + window.location.host + cdcFunctions.getMondrianCacheCleanServiceUrl();
...
cdcFunctions.getDefinitionRequest = function (map, name) {
    return cdcFunctions.makeRequest( cdcFunctions.getHazelcastConfigurationServiceUrl() + 'getMapOption',  {map: map, name: name});
};

cdcFunctions.setDefinitionRequest = function(map, name, value) {
    return cdcFunctions.makeRequest( cdcFunctions.getHazelcastConfigurationServiceUrl() + 'setMapOption',  {map: map, name: name, value: value});

};

cdcFunctions.isRunningFallbackCall = function () {

    return cdcFunctions.makeRequest( cdcFunctions.getHazelcastMonitorServiceUrl() + 'isRunningFallback',  {});
}
...
cdcFunctions.getMaxSizePolicies = function(){
    var result = cdcFunctions.makeRequest(cdcFunctions.getHazelcastConfigurationServiceUrl() + 'getMaxSizePolicies', {});
    Dashboards.setParameter('resultVar', result);
	if(resultVar != undefined){
...
cdcFunctions.getEvictionPolicies = function(){
    var result = cdcFunctions.makeRequest( cdcFunctions.getHazelcastConfigurationServiceUrl() + 'getEvictionPolicies', {});
    Dashboards.setParameter('resultVar', result);
	if(resultVar != undefined){
		if(resultVar.status != "OK"){
			return [];
