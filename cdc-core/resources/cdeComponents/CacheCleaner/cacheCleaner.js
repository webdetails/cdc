CacheCleanerComponent = BaseComponent.extend({
  ...
  triggerQuery: function(handler) {
    ...
    switch(this.breadcrumb.length){
      case 0: /* Nothing selected yet, get catalog list */
        cdcFunctions.cubeListing(handler);
        break;
      ...
      case 2: /* Catalog and cube selected, we need the dimensions for the cube */
        cdcFunctions.cubeStructure(catalog.value,cube.value,handler);
        break;
      ...
      default: /* Else, we're interested in the children of a specific member */
        cdcFunctions.memberStructure(catalog.value,cube.value,member.value,handler);
    }
  },
  ...
  handleResponse: function(resp) {
     ...
     */
    var result = cdcFunctions.extractResult(resp);
    var opts, next;
    if (result.catalogs) {
      this.catalogs = result.catalogs;
      opts = result.catalogs.map(function(e,i){
        return {
          index: i,
          value: e.name,
          label: e.name
    ...
    } else if (result.cubes) {
      opts = result.cubes.map(function(e,i){
        return {
          index: i,
          value: e.id,
          label: e.name
      ...
      opts = result.dimensions.map(function(e,i){
        return {
          index: i,
          value: e.name,
          label: e.name
    ...
    } else if (result.hierarchies) {
      opts = result.hierarchies.map(function(e,i){
        return {
          index: i,
  ...
  getURL: function(item) {
    ...
    urlMethod = methods[p];
    urlMethod = cdcFunctions.getGenericCdcServicesUrl() + webservice + '/' + urlMethod;
